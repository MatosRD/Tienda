...
?>
    ...
    <div class="encabezado">
        ...
        <h4> <button>Hombre</button> - Mujere - Niño</h4>  
    </div>    
    ...
    $realizar = mysqli_query($conexion, $buscar);

    if(mysqli_num_rows($realizar) > 0 ){
        while($fila = mysqli_fetch_assoc($realizar)){?>
            <div class="cont_catalogo"><img style="max-height: 300px" src="data:image/jpg;base64,<?php echo "".base64_encode($fila['imagen']); ?>" > 
            <?php  echo'<div class="nombre">';    echo $fila['nombre']; echo'</div>';
            echo'<div class="descripcion">';  echo $fila['descripcion']; echo'</div>';
            echo'<div class="precio">';  echo '$'.$fila['precio'].' RD'; echo'</div>';
            echo " <a href='carrito.php?id=".$fila['id_producto']." ' > Carrito </a>";
            echo'</div>  ';
        }
    }
